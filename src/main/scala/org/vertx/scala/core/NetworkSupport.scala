 ...
 */
trait NetworkSupport[+S <: NetworkSupport[S]] extends Any
  with Self[S]
  with AsJava {
   ...
   */
  def setSendBufferSize(size: Int): S = wrap(asJava.setSendBufferSize(size))

  /**
   * Set the receive buffer size for connections created by this instance to `size` in bytes.
   * @return a reference to this so multiple method calls can be chained together
   */
  def setReceiveBufferSize(size: Int): S = wrap(asJava.setReceiveBufferSize(size))

  /**
   * Set the reuseAddress setting for connections created by this instance to `reuse`.
   * @return a reference to this so multiple method calls can be chained together
   */
  def setReuseAddress(reuse: Boolean): S = wrap(asJava.setReuseAddress(reuse))

  /**
   * Set the trafficClass setting for connections created by this instance to `trafficClass`.
   * @return a reference to this so multiple method calls can be chained together
   */
  def setTrafficClass(trafficClass: Int): S = wrap(asJava.setTrafficClass(trafficClass))
