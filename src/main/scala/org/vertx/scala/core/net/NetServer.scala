...
final class NetServer private[scala] (val asJava: JNetServer) extends AnyVal
  with ServerSSLSupport[NetServer]
  with ServerTCPSupport[NetServer]
  with Closeable {

  override type J = JNetServer
  override protected[this] def self: NetServer = this
