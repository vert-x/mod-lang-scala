 ...
 */
final class Vertx(val asJava: JVertx) extends AnyVal {
   ...
   */
  // No extra cost incurred since it's an AnyVal. The internal instance
  // remains the same on each call, no new AnyVal will be created
  // Calling new instead of apply() because that would get called each time
  def fileSystem: FileSystem = new FileSystem(asJava.fileSystem())
  ...
  // remains the same on each call, no new AnyVal will be created
  // Calling new instead of apply() because that would get called each time
  def eventBus: EventBus = new EventBus(asJava.eventBus())
  ...
  // remains the same on each call, no new AnyVal will be created
  // Calling new instead of apply() because that would get called each time
  def sharedData: SharedData = new SharedData(asJava.sharedData())
