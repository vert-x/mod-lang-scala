...
final class AsyncFile private[scala] (val asJava: JAsyncFile) extends AnyVal
  with ReadStream[AsyncFile]
  with WriteStream[AsyncFile]
  with Closeable {

  override type J = JAsyncFile
  override protected[this] def self: AsyncFile = this
