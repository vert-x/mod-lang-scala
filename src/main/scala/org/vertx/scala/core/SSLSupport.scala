...
trait SSLSupport[+S <: SSLSupport[S]] extends Any
  with Self[S]
  with AsJava {
   ...
   */
  def setSSL(ssl: Boolean): S = wrap(asJava.setSSL(ssl))
   ...
   * @return A reference to this, so multiple invocations can be chained together.
   */
  def setKeyStorePath(path: String): S = wrap(asJava.setKeyStorePath(path))
   ...
   * @return A reference to this, so multiple invocations can be chained together.
   */
  def setKeyStorePassword(pwd: String): S = wrap(asJava.setKeyStorePassword(pwd))
   ...
   * @return A reference to this, so multiple invocations can be chained together.
   */
  def setTrustStorePath(path: String): S = wrap(asJava.setTrustStorePath(path))
   ...
   * @return A reference to this, so multiple invocations can be chained together.
   */
  def setTrustStorePassword(pwd: String): S = wrap(asJava.setTrustStorePassword(pwd))
