...
final class DatagramSocket private[scala] (val asJava: JDatagramSocket) extends AnyVal
  with DrainSupport[DatagramSocket]
  with NetworkSupport[DatagramSocket]
  with ReadSupport[DatagramSocket, DatagramPacket]
  with Closeable {

  override type J = JDatagramSocket
  override protected[this] def self: DatagramSocket = this
   ...
   */
  override def dataHandler(handler: DatagramPacket => Unit): DatagramSocket =
