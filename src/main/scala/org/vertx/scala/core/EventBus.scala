...
class EventBus(internal: JEventBus) {
  ...
  def registerHandler(handler: (Message[Any]) => Unit):String = internal.registerHandler(new FunctionHandler1(handler))
  ...
  def unregisterHandlerId(address: String):Unit = internal.unregisterHandler(address)
...
// FIXME creating a new FunctionHandler here will likely result in a failure 
  def unregisterHandler(address: String, handler: (Message[Any]) => Unit, resultHandler: () => Unit):Unit = {
    internal.unregisterHandler(address, new FunctionHandler1(handler), new FunctionAsyncResultHandler0(resultHandler))
  }
